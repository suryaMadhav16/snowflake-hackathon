...
class ResultsDisplay:
    ...
    def _render_progress(self, df: pd.DataFrame) -> None:
    ...
    def render_results_summary(self, results: List[CrawlResult], is_final: bool = False) -> None:
        ...
        with self.progress_container:
            ...
            # During processing, only show progress bar
            if not is_final:
                success_rate = len(df[df['success']]) / len(df)
                st.progress(success_rate, text=f"Overall Success Rate: {success_rate:.1%}")
            else:
                # Show complete visualization at the end
                self._render_progress(df)
        
        # Only show details on final update or in expander
        if is_final:
    ...
    def render_error_analysis(self, results: List[CrawlResult]) -> None:
    ...
    def clear_results(self) -> None:
        """Clear current results and reset containers"""
        st.session_state.current_results = []
        st.session_state.result_counter = 0
